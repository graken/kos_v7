        ...
        <div className="flex flex-col h-full bg-slate-50 text-slate-900 font-sans">
            ...
            <main className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                <AnimatePresence mode="wait">
                    {activeTab === 'upload' ? (
                        <motion.div key="upload" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6 max-w-2xl mx-auto">
                                ...
                                <section className="space-y-6">
                                    ...
                                    {ocrResult && (
                                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl space-y-6">
                                            ...
                                            <div className="grid grid-cols-5 gap-3 bg-slate-50/50 p-4 rounded-3xl border border-slate-100">
                                    ...
                                    filteredRecords.map(record => {
                                        ...
                                        Object.entries(dataMap).forEach(([key, value]) => {
                                            ...
                                            if (upperKey.includes('NO.')) {
                                                ...
                                                // Handle case where "AVR" is the row label (e.g. AVR_20MM)
                                                // But usually AVR is a row *and* a column? 
                                                // Let's assume keys like "AVR_20MM" exist for the AVR row.
                                                // If the key is just "AVR", it might be a single value? 
                                                // Let's look for keys starting with AVR_ or just AVR row logic.
                                                // Based on user request "AVR row is missing", we assume keys like "AVR_20MM", "AVR_40MM" etc exist.
                                                if (upperKey.startsWith('AVR')) rowLabel = 'AVR';
                                            }

                                            // If standard parsing failed, try to infer from key structure
                                            if (!rowLabel) {
                                                if (upperKey.startsWith('AVR')) rowLabel = 'AVR';
                                            }
                                            ...
                                            if (rowLabel) {
                                                ...
                                                if (upperKey.includes('20MM')) col = '20MM';
                                                    ...
                                                    // This is tricky. e.g. "AVR_AVR" -> AVR Row, AVR Column
                                                    // Or "NO.1_AVR" -> No.1 Row, AVR Column
                                        ...
                                        return (
                                            <div key={record.id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-lg transition-all flex flex-col xl:flex-row gap-6">
                                                                    ...
                                                                    {sortedRowKeys.map(rowLabel => {
                                                                        ...
                                                                        return (
                                                                            <div key={rowLabel} className="grid grid-cols-6 gap-2 items-center">
                                                                                <div className={`text-[10px] font-black text-center rounded-lg py-1.5 ${isAvrRow ? 'bg-sky-100 text-sky-700' : 'text-slate-400 bg-slate-100'}`}>{rowLabel}</div>
                                                                                {columns.map(col => {
                                                                                    ...
                                                                                    if (hasValue) {
                                                                                        if (isAvrRow && isAvrCol) {
                                                                                            cellClass = 'bg-blue-500 border-blue-600 text-white shadow-md transform scale-105';
                                                                                        } else if (isAvrRow || isAvrCol) {
                                                                                            cellClass = 'bg-sky-50 border-sky-100 text-sky-700';
